...
class Webpack5RecommendConfig {
  constructor(env, argv) {
    ...
    this.enableThread = true // 是否启用多线程
    this.enableBalbel = this.isProduction // 生产环境启用babel做编译
    this.config = {}
  }
  ...
  buildBasic() {
    ...
    if (!this.isProduction) {
      this.config.target = 'web' // 默认值：browserslist
    }
  ...
  buildDevServer() {
    ...
    this.config.devServer = {
      ...
      host: '0.0.0.0',
      liveReload: true,
      hot: true, // HMR（Hot Module Replacement），JS文件内需要调用accept()
  ...
  buildRules() {
     ...
     * 添加js解析
     */
    let jsUse = this.enableBalbel ? ['babel-loader'] : []
     ...
     * 添加jsx解析
     */
    let jsxUse = this.enableBalbel ? ['babel-loader'] : []
    if (this.enableThread) {
    ...
    if (this.isTsProject) {
       ...
       * 添加ts/tsx解析
       */
      let tsUse = this.enableBalbel ? ['babel-loader'] : []
      tsUse.push({
        loader: 'ts-loader',
        options: {
          ...
          transpileOnly: true,
          compilerOptions: {
            jsx: this.isProduction ? 'preserve' : 'react-jsxdev',
            noEmit: true
          }
        }
      })
